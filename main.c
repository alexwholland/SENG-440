...
ycc_meta_data* ycc_to_meta(ycc_data* inData, int height, int width) {
    ...
    for (j = 0; j < height / 2; j++) {
        ...
        yccMetaData->data[offset + i] = downsample_ycc(&inData->data[tracer], &inData->data[tracer + 1],
                                                        &inData->data[tracer + width], &inData->data[tracer + 1 + width]);
        }
    }
    return yccMetaData;
}
...
void save_ycc_image(ycc_data* yccData, int height, int width) {
    FILE* yccFile = fopen("ycc_image.ycc", "wb");
    if (yccFile == NULL) {
        printf("Error creating YCC image file.\n");
        exit(1);
    }

    fwrite(yccData->data, sizeof(ycc_pixel), height * width, yccFile);
...
int main(int argc, char* argv[]) {
    ...
    ycc_data* yccImage = rgb_to_ycc(inData, height, width);
    save_ycc_image(yccImage, height, width);
